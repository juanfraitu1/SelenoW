...
gse_kgg <- function(kegg_gene_list){
  gseKEGG(geneList= kegg_gene_list, organism= "mmu", maxGSSize    = 400, pvalueCutoff = 0.05,pAdjustMethod = "BH",by = "fgsea",eps = 0)
}
...
ddsl=DESeq(dds, test = "LRT",reduced = ~time)
ddsl =dds[rowSums(counts(dds))>5,]
sizeFactors(ddsl)
...
res_table=as.data.frame(results(ddsl,contrast=list(c("time_24_vs_0","time_4_vs_0","time_8_vs_0","genotypeKO.time24","genotypeKO.time4","genotypeKO.time8"))))

...
colData=as_tibble(cbind(colnames(countData),factor(treatment,levels = c("WT","KO")),factor(timee,levels=c("0","4","8","24"))))
...
resultsNames(dds)
res_table=results(dds, contrast=list(c("timee_2_vs_1","timee_3_vs_1"    ,"timee_4_vs_1","treatment_2_vs_1","timee2.treatment2","timee3.treatment2","timee4.treatment2")))
...
sig <- res_table_tb[which(res_table_tb[,7]<0.05 & abs(res_table_tb[,3])>2),]
...
kegg=dotplot(kgg , showCategory=25, split=".sign", x="NES", decreasing = TRUE)

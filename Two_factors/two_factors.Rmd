...
res_table=results(dds, contrast=list(c("group_KO24_vs_KO0","group_KO4_vs_KO0","group_KO8_vs_KO0","group_WT0_vs_KO0","group_WT24_vs_KO0","group_WT4_vs_KO0","group_WT8_vs_KO0")))
...
up_sig=as.tibble(head(sig, 50))

down_sig=as.tibble(tail(sig,50))

both=list(up_sig, down_sig)

summ=reduce(both, bind_rows)

write.csv(summ, "summary_up_down.csv")
...
```{r}
phea=pheatmap(norm_sig, scale = "row", clustering_distance_rows = "correlation", fontsize = 3, color = colorRampPalette(brewer.pal(9, "YlOrBr"))(255), cellwidth = 12.0, main = "All times", cluster_cols = TRUE)
...
#to not make a mega unentelligible heatmap, I decided to take a random sample of 300 genes, this preserves the natural clustering and avoids biases
phea=pheatmap(slice_sample(norm_sig,n = 300), scale = "row", clustering_distance_rows = "correlation", fontsize = 3, color = colorRampPalette(brewer.pal(9, "YlOrBr"))(255), cellwidth = 12.0, main = "All times", cluster_cols = TRUE)
...
a=lapply(1:length(gse$Description), function(i) {gseaplot(gse, by = "all", title = gse$Description[i], geneSetID = i)})
...
pathwayss <- lapply(1:10, function(x) {
